...
class TypeOperatorLowering(val context: JsIrBackendContext) : FunctionLoweringPass {
    ...
    override fun lower(irFunction: IrFunction) {
        irFunction.transformChildren(object : IrElementTransformer<IrFunction> {
            override fun visitFunction(declaration: IrFunction, data: IrFunction) = super.visitFunction(declaration, declaration)
            ...
            private fun lowerImplicitNotNull(expression: IrTypeOperatorCall, function: IrFunction): IrExpression {
                ...
                val argument = cacheValue(expression.argument, newStatements, function.symbol)
            ...
            private fun lowerCast(expression: IrTypeOperatorCall, function: IrFunction, isSafe: Boolean): IrExpression {
            ...
            fun lowerInstanceOf(expression: IrTypeOperatorCall, function: IrFunction, inverted: Boolean): IrExpression {
                ...
                val argument = if (isCopyRequired) cacheValue(expression.argument, newStatements, function.symbol) else expression.argument
                val check = generateTypeCheck(argument, toType)
            ...
            private fun cacheValue(value: IrExpression, newStatements: MutableList<IrStatement>, f: IrFunctionSymbol): IrExpression {
                val varSymbol = JsSymbolBuilder.buildTempVar(f, value.type, mutable = false)
            ...
            private fun lowerIntegerCoercion(expression: IrTypeOperatorCall, function: IrFunction): IrExpression {
                ...
                val argument = if (isNullable) cacheValue(expression.argument, newStatements, function.symbol) else expression.argument
